...
namespace Stimpi
{
	...
	{
		AssetHandleB iconAsset =  AssetManagerB::GetAssetNoRefCount<Texture>(s_Config.m_ResourcePath / name);
		return AssetManagerB::GetAsset(iconAsset).As<Texture>();
	}

	ImTextureID EditorResources::GetIconTextureID(std::string name)
	{
		AssetHandleB iconAsset = AssetManagerB::GetAssetNoRefCount<Texture>(s_Config.m_ResourcePath / name);
		Texture* iconTexture = AssetManagerB::GetAsset(iconAsset).As<Texture>();
