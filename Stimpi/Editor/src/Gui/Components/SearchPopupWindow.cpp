#include "stpch.h"
#include "Gui/Components/SearchPopupWindow.h"
...
namespace Stimpi
{
	...
	bool SearchPopupWindow::OnImGuiRender(const char* name, bool* show, std::vector<std::string>& data)
	{
		...
		ImGui::Separator();
		std::string popupChName = fmt::format("{} ##SearchPopupW", name);
		ImGui::BeginChild(popupChName.c_str());
		for (auto name : data)
		{
	...
	std::string SearchPopupWindow::GetSelection()
	{
