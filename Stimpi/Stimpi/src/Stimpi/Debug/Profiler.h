	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				if (Log::GetCoreLogger()) // BeginSession() might be before Log::Init()
				{
					ST_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->m_Name);
				}
				InternalEndSession();
			}
...
#define ST_PROFILE_END_SESSION() ::Stimpi::Instrumentor::Get().EndSession()
#define ST_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Stimpi::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Stimpi::InstrumentationTimer timer##line(fixedName##line.Data)
...
#define ST_PROFILE_FUNCTION() ST_PROFILE_SCOPE(__FUNCTION__)
#else
