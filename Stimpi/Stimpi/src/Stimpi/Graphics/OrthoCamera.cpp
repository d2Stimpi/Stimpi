...
namespace Stimpi
{
	OrthoCamera::OrthoCamera(float left, float right, float bottom, float top)
		: m_ProjectionMatrix(glm::ortho(left, right, bottom, top, -10.0f, 1000.0f)), m_ViewMatrix(1.0f), m_Positin(0.0f)
	{
	...
	{
		m_ViewQuad = glm::vec4(left, right, bottom, top);
		m_ProjectionMatrix = glm::ortho(left, right, bottom, top, -10.0f, 1000.0f);
		RecalcViewProjection();
	}
